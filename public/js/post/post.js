    ...
    if (searchInput) {
        ...
        searchInput.addEventListener('input', function() {
            ...
            searchTimeout = setTimeout(() => {
                ...
                rows.forEach(row => {
                    const titleCell = row.querySelector('td:first-child a')?.textContent.toLowerCase() || '';
                    const descriptionCell = row.querySelector('td:nth-child(2) div')?.textContent.toLowerCase() || '';
                    
                    if (titleCell.includes(searchTerm) || descriptionCell.includes(searchTerm)) {
                        row.style.display = '';
                    } else {
...
function savePost() {
    ...
    // Get form values
    const announcementTitle = document.getElementById('announcement_title').value.trim();
    const description = document.getElementById('description').value.trim();
    const status = document.getElementById('status').value;
    
    // Basic validation
    if (!announcementTitle) {
        showError('announcement_title', 'Announcement title is required');
        return;
    }
    ...
    // Show loading state
    const saveBtn = document.querySelector('#add-post-modal .btn-primary');
    ...
    .then(data => {
        if (data.success) {
            ...
            if (data.errors) {
                ...
                showErrorMessage(data.message || 'Failed to save post');
            }
        }
    })
    .catch(error => {
        console.error('Error:', error);
        showErrorMessage('An error occurred while saving the post');
    })
    .finally(() => {
        // Reset button state
...
function clearErrors() {
    ...
    const inputs = document.querySelectorAll('#add-post-modal .form-control, #add-post-modal .form-select');
...
function editPost(postId) {
    ...
    .then(data => {
        if (data.success) {
            ...
            document.getElementById('edit_announcement_title').value = data.post.announcement_title;
            document.getElementById('edit_description').value = data.post.description || '';
            document.getElementById('edit_status').value = data.post.status;
            ...
            showErrorMessage(data.message || 'Failed to load post data');
        }
    })
    .catch(error => {
        console.error('Error:', error);
        showErrorMessage('An error occurred while loading post data');
    });
}
...
function updatePost() {
    ...
    const announcementTitle = document.getElementById('edit_announcement_title').value.trim();
    const description = document.getElementById('edit_description').value.trim();
    const status = document.getElementById('edit_status').value;
    
    // Basic validation
    if (!announcementTitle) {
        showEditError('edit_announcement_title', 'Announcement title is required');
        return;
    }
    ...
    // Show loading state
    const updateBtn = document.querySelector('#edit-post-modal .btn-primary');
    ...
    .then(data => {
        if (data.success) {
            ...
            if (data.errors) {
                ...
                showErrorMessage(data.message || 'Failed to update post');
            }
        }
    })
    .catch(error => {
        console.error('Error:', error);
        showErrorMessage('An error occurred while updating the post');
    })
    .finally(() => {
        // Reset button state
...
function clearEditErrors() {
    const errorElements = document.querySelectorAll('#edit-post-modal [id$="_error"]');
    errorElements.forEach(element => {
        element.textContent = '';
        element.style.display = 'none';
    });
    ...
    // Remove error classes from inputs
    const inputs = document.querySelectorAll('#edit-post-modal .form-control, #edit-post-modal .form-select');
    inputs.forEach(input => {
        input.classList.remove('border-danger');
    });
}
...
function resetEditForm() {
    ...
    document.getElementById('edit_announcement_title').value = '';
...
function performDelete(postId) {
    ...
    .then(data => {
        if (data.success) {
            // Show success message
            showDeleteSuccessMessage('Post deleted successfully');
...
// Refresh posts table
function refreshPostsTable(page = 1, perPage = 10) {
    const url = `/post/get-posts?page=${page}&per_page=${perPage}`;
    ...
    .then(data => {
        if (data.success) {
            updatePostsTable(data.posts, data.pagination);
        }
    })
    .catch(error => {
...
function updatePostsTable(posts, pagination) {
    const tableBody = document.getElementById('posts-table-body');
    if (!tableBody) return;
    ...
    if (posts.length === 0) {
        html = '<tr><td colspan="5" class="text-center py-8 text-slate-500">No posts found</td></tr>';
    } else {
        posts.forEach(post => {
            const statusClass = post.status === 'active' ? 'text-success' : 'text-danger';
            ...
            html += `
                <tr class="intro-x">
                    <td>
                        <a href="" class="font-medium whitespace-nowrap">${post.announcement_title}</a>
                    </td>
                    <td>
                    ...
                    <td class="table-report__action w-56">
                        <div class="flex justify-center items-center">
                            <a class="flex items-center mr-3" href="javascript:;" onclick="editPost(${post.id})">
                            ...
                            <a class="flex items-center text-danger" href="javascript:;" onclick="deletePost(${post.id})">
    ...
    // Update post count
    updatePostCount(posts.length, pagination.current_page, pagination.per_page);
}

// Update post count display with pagination info
function updatePostCount(count, currentPage, currentPerPage) {
    ...
    if (countElement) {
        if (count > 0) {
            ...
            countElement.textContent = `Showing ${start} to ${end} posts`;
        } else {
...
function initializePagination() {
    // This function can be used to initialize pagination controls if needed
    // The pagination component should handle most of the functionality
}
