    ...
    if (searchInput) {
        ...
        searchInput.addEventListener('input', function() {
            ...
            searchTimeout = setTimeout(() => {
                ...
                rows.forEach(row => {
                    const departmentName = row.querySelector('td:first-child a')?.textContent.toLowerCase() || '';
...
function saveDepartment() {
    ...
    // Basic validation
    if (!departmentName) {
        showError('department_name', 'Department name is required');
        return;
    }
    ...
    // Show loading state
    const saveBtn = document.querySelector('#add-department-modal .btn-primary');
    ...
    .then(data => {
        if (data.success) {
            ...
            if (data.errors) {
                ...
                showErrorMessage(data.message || 'Failed to save department');
            }
        }
    })
    .catch(error => {
        console.error('Error:', error);
        showErrorMessage('An error occurred while saving the department');
    })
    .finally(() => {
        // Reset button state
...
function clearErrors() {
    ...
    const inputs = document.querySelectorAll('#add-department-modal .form-control, #add-department-modal .form-select');
...
function editDepartment(departmentId) {
    ...
    fetch(`/department-management/${departmentId}/edit`, {
        method: 'GET',
        headers: {
            'Accept': 'application/json',
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || ''
    ...
    .then(data => {
        if (data.success) {
            ...
            document.getElementById('edit_department_name').value = data.department.department_name;
            ...
            showErrorMessage(data.message || 'Failed to load department data');
        }
    })
    .catch(error => {
        console.error('Error:', error);
        showErrorMessage('An error occurred while loading department data');
    });
}
...
function updateDepartment() {
    ...
    const departmentName = document.getElementById('edit_department_name').value.trim();
    const description = document.getElementById('edit_description').value.trim();
    const status = document.getElementById('edit_status').value;
    
    // Basic validation
    if (!departmentName) {
        showEditError('edit_department_name', 'Department name is required');
        return;
    }
    ...
    // Show loading state
    const updateBtn = document.querySelector('#edit-department-modal .btn-primary');
    ...
    // Send AJAX request
    fetch(`/department-management/${departmentId}`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '',
    ...
    .then(data => {
        if (data.success) {
            ...
            if (data.errors) {
                ...
                showErrorMessage(data.message || 'Failed to update department');
            }
        }
    })
    .catch(error => {
        console.error('Error:', error);
        showErrorMessage('An error occurred while updating the department');
    })
    .finally(() => {
        // Reset button state
...
function clearEditErrors() {
    const errorElements = document.querySelectorAll('#edit-department-modal [id$="_error"]');
    errorElements.forEach(element => {
        element.textContent = '';
        element.style.display = 'none';
    });
    ...
    // Remove error classes from inputs
    const inputs = document.querySelectorAll('#edit-department-modal .form-control, #edit-department-modal .form-select');
    inputs.forEach(input => {
        input.classList.remove('border-danger');
    });
}
...
function deleteDepartment(departmentId) {
    // Store the department ID for deletion
    window.departmentToDelete = departmentId;
...
function initializeDeleteConfirmation() {
    ...
    if (confirmDeleteBtn) {
        confirmDeleteBtn.addEventListener('click', function() {
            const departmentId = window.departmentToDelete;
            if (departmentId) {
                performDelete(departmentId);
            }
        });
    }
}
...
function performDelete(departmentId) {
    // Show loading state
    ...
    .then(data => {
        if (data.success) {
            // Show success message
            showDeleteSuccessMessage('Department deleted successfully');
            ...
            showErrorMessage(data.message || 'Failed to delete department');
        }
    })
    .catch(error => {
        console.error('Error:', error);
        showErrorMessage('An error occurred while deleting the department');
    })
    .finally(() => {
        // Reset button state
...
function refreshDepartmentsTable(page = 1, perPage = 10) {
    const url = `/department-management/departments?page=${page}&per_page=${perPage}`;
    ...
    .then(data => {
        if (data.success) {
            updateDepartmentsTable(data.departments, data.pagination);
        }
    })
    .catch(error => {
...
// Update departments table with new data
function updateDepartmentsTable(departments, pagination) {
    const tableBody = document.getElementById('departments-table-body');
    if (!tableBody) return;
    ...
    if (departments.length === 0) {
        html = '<tr><td colspan="5" class="text-center py-8 text-slate-500">No departments found</td></tr>';
    } else {
        departments.forEach(department => {
            const statusClass = department.status === 'active' ? 'text-success' : 'text-danger';
            const statusIcon = department.status === 'active' 
            ...
            const createdDate = new Date(department.created_at).toLocaleDateString('en-US', {
                month: 'short',
                day: 'numeric',
                year: 'numeric'
            });
    ...
    updateDepartmentCount(departments.length, pagination.current_page, pagination.per_page);
}

// Update department count display with pagination info
function updateDepartmentCount(count, currentPage, currentPerPage) {
    ...
    if (countElement) {
        if (count > 0) {
            ...
            countElement.textContent = `Showing ${start} to ${end} departments`;
        } else {
            countElement.textContent = 'No departments found';
        }
    }
}
